...
class GreenhouseSensorSimulator:
    ...
    """محاكي بيانات أجهزة الاستشعار للصوبة الزراعية مع تحسينات واقعية"""
    def __init__(self, location="Cairo, Egypt", output_dir="."):
        ...
        #إعداد أسماء الملفات مع المسار الجديد
        ...
        # إنشاء ملف CSV مع العناوين في المسار الصحيح
    ...
    def display_data(self, data):
        ...
        print("\n" + "="*70)
        print(f"🌱 بيانات أجهزة الاستشعار - الصوبة الزراعية")
        print("="*70)
    ...
    def run_infinite(self, interval_seconds=5):
        ...
        print(f"🚀 بدء محاكاة أجهزة الاستشعار...")
        print(f"⏱️ التشغيل: مستمر | فترة القراءة: {interval_seconds} ثانية")
        print(f"💾 حفظ البيانات في: {os.path.abspath(self.output_dir)}") # عرض المسار الكامل
        ...
        try:
            ...
            print(f"📁 البيانات محفوظة في:")
            print(f"  - {self.csv_filename}")
            print(f"  - {self.json_filename}")

# --- التعديل الرئيسي هنا ---
def main():
    """الدالة الرئيسية - تشغيل مستمر تلقائياً"""
    print("🌱 مرحباً بك في محاكي أجهزة الاستشعار للصوبة الزراعية")
    print("="*60)
    print("🔄 التشغيل في الوضع المستمر تلقائياً")
    print("⏱️ قراءة كل 5 ثوان")
    print("📊 بيانات واقعية مع تغيرات تدريجية بسيطة")
    
    # === قم بتحديد المسار المطلوب هنا ===
    # استخدام r"..." لضمان قراءة المسار بشكل صحيح على ويندوز
    save_path = r"E:\\MyCareer\\DepiData\\Data Hive\\Final_Project"
    
    # إنشاء محاكي الأجهزة مع تمرير المسار الجديد
